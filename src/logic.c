...
/**
 * Setzt die Position des Partikel mit der Kamera weiter
 */
void nextCameraParticle() {
    if (G_CameraParticlePos +1 < getParticleCnt())
        ++G_CameraParticlePos;
    else
        G_CameraParticlePos = 0;
}

/**
 * Das Partikel, auf dem die Kamera ist.
 */
Particle getCameraParticle (void)
{
    if (getParticleCnt() > 0)
        return getParticleList()[G_CameraParticlePos];
    else 
        return NULL;
}
...
void calcTimeRelatedStuff (double interval)
{
	...
	if (!G_ParticleFreeze)
    {
        calcParticleAttributesEuler (G_Spheres, SPHERE_CNT, interval*5,G_PersecutorParticle, G_CameraParticlePos);
    }
